...
function initMiddleware() {
	...
	// Try to load tokens from environment variables first
	const envAdminTokens = process.env.ADMIN_TOKENS ? process.env.ADMIN_TOKENS.split(',') : [];
	const envUploadTokens = process.env.UPLOAD_TOKENS ? process.env.UPLOAD_TOKENS.split(',') : [];
	const envViewReportTokens = process.env.VIEW_REPORT_TOKENS ? process.env.VIEW_REPORT_TOKENS.split(',') : [];

	if (envAdminTokens.length > 0 || envUploadTokens.length > 0 || envViewReportTokens.length > 0) {
		log.info('Loading tokens from environment variables.');
		envAdminTokens.forEach(token => adminToken.push({ token: token.trim() }));
		envUploadTokens.forEach(token => uploadToken.push({ token: token.trim() }));
		envViewReportTokens.forEach(token => viewReportToken.push({ token: token.trim() }));
	} else if (cliArgs.noTokenFile === false) {
	...
	if (typeof cliArgs.token !== 'string') {
		if (!loadedTokenFile && adminToken.length === 0) {
